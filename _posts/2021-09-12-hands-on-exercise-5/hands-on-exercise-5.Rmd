...
description: |
  Learning more about Network constrained Spatial Point Patterns Analysis (NetSPAA).
author:
  - name: Teo Jun Peng
...
Network constrained Spatial Point Patterns Analysis (NetSPPA) is a collection of spatial point patterns analysis methods special developed for analysing spatial point event occurs on or alongside network. The spatial point event can be locations of traffic accidents or childcare centres for example. The network, on the other hand can be a road network or river network.

This hands-on exercise enables us to gain hands-on experience on using appropriate functions of spNetwork package:
...
```{r}
packages = c('sp', 'rgdal', 'spNetwork', 'tmap', 'FNN')
for (p in packages){
...
The code chunk below uses readOGR() of rgdal package to import Punggol_St and Punggol_CC geospatial data sets into RStudio as SpatialLinesDataFrame and SpatialPointsDataFrame respectively.

```{r}
network <- readOGR(dsn="handson5data/geospatial", 
                   layer="Punggol_St",
                   verbose = FALSE)
childcare <- readOGR(dsn="handson5data/geospatial",
                     layer="Punggol_CC",
                     verbose = FALSE)
```
...
```{r}
densities <- spNetwork::nkde(network, 
                        events = childcare,
