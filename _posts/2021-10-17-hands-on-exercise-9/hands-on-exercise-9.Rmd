...
description: |
  Learning how to calibrate geographically weighted regression models by using GWmodel package of R.
author:
  - name: Teo Jun Peng
...
**GWmodel** package provides a collection of localised spatial statistical methods, namely: GW summary statistics, GW principal components analysis, GW discriminant analysis and various forms of GW regression; some of which are provided in basic and robust (outlier resistant) forms. 
...
The geospatial data used in this hands-on exercise is called MP14_SUBZONE_WEB_PL, and is in ESRI shapefile format. The shapefile consists of URA Master Plan 2014’s planning subzone boundaries. Polygon features are used to represent these geographic boundaries. The GIS data is in svy21 projected coordinates systems.
...
The EPSG has been changed to **3414** now, which means it worked **successfully.**

We will now reveal the extent of *mpsz_svy21* by using *st_bbox()* of **sf** package.
...
The *condo_resale_2015* is in csv file format. The code chunk below uses *read_csv()* function of **readr** package to import *condo_resale_2015* into R as a tibble data frame called *condo_resale*.
...
The code chunks below uses *glimpse()* to display the data structure of will do the job.
...
Currently, the condo_resale data frame is aspatial. We will convert it to a *sf* object. The code chunk below converts condo_resale data frame into a simple feature data frame by using *st_as_sf()* of **sf** package.

*st_transform()* of **sf** package will be used to convert the coordinates from wgs84 (i.e. crs:4326) to svy21 (i.e. crs=3414).
...
Now, we can plot **LOG_SELLING_PRICE** using the code chunk below.
...
Voila! The distribution is **relatively less skewed** after the transformation.

## Multiple Histogram Plots distribution of variables

In this section, we will draw small multiple histograms (also known as treltis plot) by using *ggarrange()* of **ggpubr** package.
...
First, we will turn on the interactive mode (AKA "View" mode) of tmap by using the code chunk below. 
...
**tmap_options(check.and.fix = TRUE) is used to rectify any invalid polygons.**
...
The figure plotted above reveals that there are a few statistical outliers with relatively high selling prices.
...
Matrix reorder is very important for mining the hidden structure and pattern in the matrix. There are **four methods** in corrplot (parameter order), named “AOE”, “FPC”, “hclust”, “alphabet”. In the code chunk above, alphabet order is used. It orders the variables alphabetically.
...
With reference to the report above, it is clear that not all the independenct variables are statistically significant. We will revised the model by removing those variables which are not statistically significant.
...
In this section, we will be learning a fantastic R package specially programmed for performing OLS regression. It is called **olsrr**. 
...
In the code chunk below, the *o_ls_plot_resid_fit()* of **olsrr** package is used to perform linearity assumption test.
...
Below is a code chunk to switch back to "plot" mode before continuing our analysis.
...
Next, *nb2listw()* of **spdep** packge will be used to convert the output neighbours lists (i.e. nb) into spatial weights.
...
Since the Observed Global Moran I = 0.1424418 which is greater than 0, we can infer than the residuals resembles cluster distribution.
...
**- Because the  area is calculated as Square Metres?**
...
The code chunk used is similar to the one used to compute the fixed bandwidth, except the **adaptive** argument has changed to **TRUE**.
...
The result shows that the 30 is the recommended data point to be used.
