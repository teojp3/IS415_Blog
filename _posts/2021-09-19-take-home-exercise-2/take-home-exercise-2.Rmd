...
We will first import in data using the Airbnb (2019 and 2021) datasets, but we will only use the 2019 dataset to see the distribution of listings as we are interested in analysing the distribution **Pre and Post COVID-19**. The analysis aims to uncover whether the distribution of Airbnb listings are affected by location factors such as located near hotels or tourist attractions, so those datasets are imported in as well. 
...
The coordinates in the respective datasets were defined in values of Latitude and Longitude, therefore being congruent with **EPSG 4326**. However, Singapore's national Project Coordination Systems should be **EPSG 3414 (SVY21)** instead, therefore we will transform the *crs* to EPSG 3414.
...
### 3.1.1 Importing in Geospatial Data 
...
mrt_sf <- st_read(dsn = "data/the2data/MRT", 
                layer = "MRTLRTStnPtt")
```
Projection of **mrt_sf** is in SVY21, so we are good to go.
