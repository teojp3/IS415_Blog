---
title: "Spatio-temporal Patterns of COVID-19 in Jakarta, Indonesia"
description: |
...
```{r results='hide'}
file_list <- list.files(path = "the1data/COVID-DATA" , pattern = "*.xlsx",  full.names = T)
...
```{r}
df2 <- df2 %>% 
  mutate_at("MonthYear", str_replace, "the1data/COVID-DATA/Standar Kelurahan Data Corona", "")
...
```{r include=FALSE}
aspatial_df <- write_rds(final_df, "the1data/rds/aspatial_df.rds")
```

```{r}
aspatial_df <- read_rds("the1data/rds/aspatial_df.rds")
```
...
```{r}
geospatial_df <- st_read(dsn = "the1data/BATAS DESA DESEMBER 2019 DUKCAPIL DKI JAKARTA", 
...
```{r}
Case_Rate_rds <- write_rds(Case_Rate, "the1data/rds/Case_Rate_rds.rds")
Death_Rate_rds <- write_rds(Death_Rate, "the1data/rds/Death_Rate_rds.rds")
```

```{r}
Case_Rate_rds <- read_rds("the1data/rds/Case_Rate_rds.rds")
Death_Rate_rds <- read_rds("the1data/rds/Death_Rate_rds.rds")
```
...
Sum_cases <- sum(Death_Rate_Final$POSITIVE_CASES)
Sum_population <- sum(Death_Rate_Final$JUMLAH_PEN)

Case_reference_rate <- Sum_cases / Sum_population
Death_reference_rate <- Sum_deaths / Sum_population
```
...
```{r}
Case_RR_Map <- tm_shape(Death_Rate_Final) +
     tm_fill("CaseSMR", title="Case Risk SMR", style = 'jenks', 
             palette = "Reds")  +
  tm_borders() +
...
```{r}
Death_RR_Map <- tm_shape(Death_Rate_Final) +
     tm_fill("DeathSMR", title="Death Risk SMR", style = 'jenks', 
             palette = "-RdBu")  +
  tm_borders() +
  tm_layout(legend.outside = TRUE, legend.outside.position = "right") +
  tm_compass(type="8star", size = 2, position = c( 'left','bottom')) +
