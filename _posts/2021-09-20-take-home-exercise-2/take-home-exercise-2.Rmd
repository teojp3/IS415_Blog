...
- If they are NOT spatially independent, what location factors (Hotels, Hawker Centres, MRT Stations, Tourist Attractions) were they attracted to?
...
kde_airbnb2021.km <- rescale(airbnb_2021_ppp, 1000, "km")
...
kde_airbnb2021.bw  <- density(kde_airbnb2021.km, sigma=1, edge=TRUE, kernel="gaussian")
...
gridded_kde_airbnb2021_bw <- as.SpatialGridDataFrame.im(kde_airbnb2021.bw)
...
kde_airbnb_2021_bw_raster <- raster(gridded_kde_airbnb2021_bw)
...
projection(kde_airbnb_2021_bw_raster) <- CRS("+init=EPSG:3414 +datum=WGS84 +units=km")
...
```
The plot above reveals that the distribution of Airbnb Listings and Hotels are not spatially independent. Since the observed(black) line is outside the envelope, the null hypothesis is rejected because the empirical k-cross line is not within the enveloper of the 99.9% confidence interval. 
...
The plot above reveals that the distribution of Airbnb Listings and Tourist Attractions are not spatially independent. Since the observed(black) line is outside the envelope, the null hypothesis is rejected because the empirical k-cross line is not within the enveloper of the 99.9% confidence interval. 
...
The plot above reveals that the distribution of Airbnb Listings and Hawker Centres are not spatially independent. Since the observed(black) line is outside the envelope, the null hypothesis is rejected because the empirical k-cross line is not within the enveloper of the 99.9% confidence interval. 
