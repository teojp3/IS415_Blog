...
- Road Network of Punggol, Singapore (Taken from a previous Hands-on Exercise)
...
- [Taxi Stops](https://datamall.lta.gov.sg/content/datamall/en.html)
...
The Bus Stops data set is taken from DataMall, which is under the official website of the Land Transport Authority (LTA) of Singapore.
...
We will be using *three factors* to test out their correlation and relationship with the location of bus stops. The three factors are: **1) Schools, 2) MRT/LRT Stations, 3) Taxi Stops**
...
# Malls <- read_csv("data/Malls.csv")
# 
# Malls <- st_as_sf(Malls, coords = c("Longitude", "Latitude"), crs = 4326) %>%
#     st_transform(crs = 3414)
```
...
Taxi <- readOGR(dsn="data/TaxiStop", 
                   layer="TaxiStop_P",
                   verbose = FALSE)

Taxi <- spTransform(Taxi,CRS("+init=epsg:3414"))
```
...
```{r}
lixels <- lixelize_lines(network,1000,mindist = 500)
```
...
**Ho: The observed spatial point events (i.e distribution of Bus Stops) are uniformly distributed over the road network in Punggol Planning Area located in Singapore.**

```{r eval=FALSE}
kfun_busstops <- kfunctions(network, 
                           Bus,
                           start = 0, 
                           end = 1000, 
                           step = 50, 
